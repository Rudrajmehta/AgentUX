...
2. Task-aware: "find pricing" in 1 click = great. "audit everything" with 3/35 = poor.
3. Shallow runs score low — you can't prove usability in 2 steps
4. Every score includes actionable recommendations
"""
...
def _is_targeted_task(task: str) -> bool:
    """Heuristic: is this a specific task or a general audit?"""
    task_lower = task.lower()
    # General audit indicators
    audit_words = ["explore", "audit", "discover", "assess", "evaluate", "report what"]
    return not any(w in task_lower for w in audit_words)


def compute_discoverability(trace: RunTrace) -> ScoreResult:
    """How easily the agent found what it needed.

    Two modes:
    - Targeted task ("find pricing"): Did the agent find the right thing quickly?
      Score based on: task success + speed + directness
    - General audit ("explore this CLI"): What fraction of affordances were tested?
      Score based on: interaction coverage
    """
    ...
    if total == 0:
        return ScoreResult(
            name="Discoverability", value=0.0,
            explanation="No affordances on this surface.",
            inputs={"recommendations": ["Surface has no discoverable elements"]},
        )

    targeted = _is_targeted_task(trace.task)

    if targeted:
        # TARGETED TASK: score based on task outcome + efficiency
        # Did the agent find what it needed?
        score = 0.0

        # Component 1: Task success (0-40)
        if trace.success:
            score += 40
        elif trace.step_count > 0:
            score += 10  # tried but failed

        # Component 2: Speed — fewer steps to find target = better (0-30)
        if trace.step_count > 0:
            # 1-3 steps = full speed bonus, degrades after that
            speed = max(0, 30 - (trace.step_count - 1) * 5)
            score += speed

        # Component 3: Directness — did the agent go straight there? (0-20)
        backtracks = sum(1 for s in trace.steps if s.action_type == "back")
        errors = sum(1 for s in trace.steps if not s.success)
        directness = max(0, 20 - backtracks * 8 - errors * 5)
        score += directness

        # Component 4: Did the agent actually interact with something? (0-10)
        if len(interacted) > 0:
            score += 10

        explanation = (
            f"{'Found target' if trace.success else 'Did not find target'} "
            f"in {trace.step_count} steps, {backtracks} backtracks, "
            f"{len(interacted)} affordances used."
        )

        if not trace.success:
            recommendations.append("Agent could not find the target — check if the information is accessible from the main navigation")
        if trace.step_count > 5:
            recommendations.append(f"Took {trace.step_count} steps to find target — consider making this more prominent")

    else:
        # GENERAL AUDIT: score based on coverage
        score = interaction_rate * 100

        if interaction_rate < 0.2 and total_relevant > 3:
            score = min(score, 40)
            recommendations.append(
                f"Agent only explored {len(interacted)}/{total_relevant} "
                f"affordances ({interaction_rate:.0%})"
            )

        explanation = (
            f"Explored {len(interacted)}/{total_relevant} "
            f"affordances ({interaction_rate:.0%})."
        )

        # List untested affordances
        missed = [a for a in relevant if a.status != AffordanceStatus.INTERACTED]
        if missed and len(missed) > 2:
            names = [a.name for a in missed[:5]]
            recommendations.append(f"Not tested: {', '.join(names)}")

    # Bonus: early discovery
    first_step = None
    for step in trace.steps:
        if step.affordances_discovered:
            first_step = step.step_number
            break
    if first_step and first_step <= 2:
        score = min(100, score + 5)

    return ScoreResult(
        name="Discoverability", value=min(100, max(0, score)),
        explanation=explanation,
        inputs={
            "interacted": len(interacted),
            "total_relevant": total_relevant,
            "interaction_rate": round(interaction_rate, 3),
            "targeted_task": targeted,
...
def compute_actionability(trace: RunTrace) -> ScoreResult:
    ...
    - Success rate (0-50)
    - First-try rate (0-25)
    - Depth bonus (0-25): more actions = more confidence
    - Capped at 60 with <3 actions
    """
    ...
    if total_actions == 0:
        recommendations.append("Agent took no actions — cannot evaluate actionability")
        return ScoreResult(
            name="Actionability", value=0.0,
    ...
    if total_actions < 3:
        score = min(score, 60)
        recommendations.append(f"Only {total_actions} action(s) tested — low confidence (capped at 60)")
    if success_rate < 0.7:
        failed = total_actions - successful
        recommendations.append(f"{failed}/{total_actions} actions failed — surface has friction")

    return ScoreResult(
        name="Actionability", value=min(100, max(0, score)),
        explanation=f"{successful}/{total_actions} succeeded, {first_try} first-try.",
        inputs={
            "successful": successful, "total_actions": total_actions,
            "first_try": first_try, "success_rate": round(success_rate, 3),
            "depth_bonus": depth_bonus, "recommendations": recommendations,
        },
    )
...
def compute_recovery(trace: RunTrace) -> ScoreResult:
    ...
    if errors_encountered == 0:
        recommendations.append("No errors occurred — recovery untested (capped at 50)")
        return ScoreResult(
            name="Recovery", value=50.0,
    ...
    if unrecoverable > 0:
        recommendations.append(f"{unrecoverable} unrecoverable error chains — surface lacks recovery guidance")

    return ScoreResult(
        name="Recovery", value=min(100, max(0, score)),
        explanation=(
        ...
        inputs={
            ...
            "dead_ends": dead_ends, "recovered": recovered,
            "unrecoverable": unrecoverable, "recommendations": recommendations,
        },
    )
...
def compute_efficiency(trace: RunTrace) -> ScoreResult:
    ...
    if total_steps < 3:
        score = min(score, 50)
        recommendations.append(f"Only {total_steps} step(s) — too few to evaluate efficiency (capped at 50)")
    if backtracks > 0:
    ...
    return ScoreResult(
        name="Efficiency", value=min(100, max(0, score)),
        explanation=f"{total_steps} steps, {backtracks} backtracks, {redundant_reads} redundant, {wasted} wasted.",
        inputs={
            "actual_steps": total_steps, "backtracks": backtracks,
            "redundant_reads": redundant_reads, "wasted_steps": wasted,
            "penalty": penalty, "recommendations": recommendations,
        },
    )
...
def compute_documentation_clarity(trace: RunTrace) -> ScoreResult:
    ...
    if total_steps == 0:
        return ScoreResult(
            name="Documentation Clarity", value=0.0,
            explanation="No steps.",
    ...
    return ScoreResult(
        name="Documentation Clarity", value=min(100, max(0, score)),
        explanation=f"{facts_count} unique facts in {total_steps} steps ({fact_density:.1f}/step).",
        inputs={
            "unique_facts": facts_count, "fact_density": round(fact_density, 2),
            "clarity_ratio": round(clarity_ratio, 2), "depth_pts": round(depth_pts, 1),
            "recommendations": recommendations,
        },
    )
...
def compute_tool_clarity(trace: RunTrace) -> ScoreResult:
    """How clear were CLI commands / MCP tools?

    Capped at 50 with <3 tool invocations.
    """
    ...
    if correct < total:
        recommendations.append(f"{total - correct}/{total} calls failed — improve schemas/descriptions")

    return ScoreResult(
        name="Tool Clarity", value=min(100, max(0, score)),
        explanation=f"{correct}/{total} correct, {arg_rate:.0%} args, {help_useful} help assists.",
        inputs={
            "correct": correct, "total": total,
            "arg_rate": round(arg_rate, 2), "help_useful": help_useful,
            "recommendations": recommendations,
        },
    )
