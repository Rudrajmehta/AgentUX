...
def generate_browser_trace(target: str = "https://example.com", days_ago: int = 0) -> RunTrace:
    ...
    sections = [
        ("header", True), ("navigation", True), ("hero", True), ("features", True),
        ("pricing", True), ("docs", False), ("footer", False), ("search", False),
        ("cta", True), ("login", False),
    ]

    for name, relevant in sections:
        status = random.choice([AffordanceStatus.DISCOVERED, AffordanceStatus.INTERACTED, AffordanceStatus.MISSED])
        if name in ("pricing", "cta", "navigation"):
            status = AffordanceStatus.INTERACTED
        trace.affordances.append(Affordance(
            name=name, kind="section", status=status, relevant=relevant,
        ))

    steps_data = [
        ("Observing the homepage structure", "extract_text", "read", True, ["Product homepage with nav, hero, features"]),
        ("Found navigation — looking for pricing link", "click", "click", True, ["Nav has: Home, Features, Pricing, Docs, Contact"]),
        ("Navigated to pricing page", "navigate", "navigate", True, ["Pricing page loaded"]),
        ("Reading pricing tiers", "extract_text", "read", True, ["Three tiers: Free, Pro ($29/mo), Enterprise (custom)"]),
        ("Looking for enterprise contact", "click", "click", True, ["Contact Sales button found"]),
        ("Task complete", "done", "done", True, ["Pricing: Free/Pro/Enterprise", "Contact: sales@example.com"]),
    ]

    for i, (thought, action, atype, success, facts) in enumerate(steps_data, 1):
        trace.add_step(StepRecord(
            step_number=i, thought_summary=thought, action=action,
            action_type=atype, result=f"Step {i} result", success=success,
            extracted_facts=facts, tokens_used=random.randint(200, 600),
...
def generate_markdown_trace(target: str = "https://example.com/llms.txt", days_ago: int = 0) -> RunTrace:
    ...
    for name in ["Introduction", "Installation", "Configuration", "Quick Start", "API Reference", "FAQ"]:
        status = random.choice([AffordanceStatus.INTERACTED, AffordanceStatus.DISCOVERED])
        trace.affordances.append(Affordance(name=name, kind="section", status=status, relevant=True))
    ...
    steps_data = [
        ...
        ("Reading Installation section", "read_section", "read", True, ["pip install example-tool"]),
        ("Reading Quick Start section", "read_section", "read", True, ["3-step quickstart provided"]),
    ...
    for i, (thought, action, atype, success, facts) in enumerate(steps_data, 1):
        trace.add_step(StepRecord(
            step_number=i, thought_summary=thought, action=action,
            action_type=atype, result=f"Step {i} result", success=success,
            extracted_facts=facts, tokens_used=random.randint(150, 400),
...
def generate_cli_trace(target: str = "uv", days_ago: int = 0) -> RunTrace:
    ...
    for name in ["init", "add", "remove", "run", "sync", "lock"]:
        status = AffordanceStatus.INTERACTED if name in ("init", "add") else AffordanceStatus.DISCOVERED
        trace.affordances.append(Affordance(name=name, kind="command", status=status, relevant=name in ("init", "add")))

    for flag in ["--help", "--version", "--name", "--python"]:
        trace.affordances.append(Affordance(name=flag, kind="flag", status=AffordanceStatus.DISCOVERED, relevant=True))

    steps_data = [
        ("Checking help to discover commands", "help", "read", True, ["Commands: init, add, remove, run, sync"]),
    ...
    for i, (thought, action, atype, success, facts) in enumerate(steps_data, 1):
        trace.add_step(StepRecord(
            step_number=i, thought_summary=thought, action=action,
            action_type=atype, result=f"Step {i} result", success=success,
            extracted_facts=facts, tokens_used=random.randint(150, 500),
...
def generate_mcp_trace(target: str = "python server.py", days_ago: int = 0) -> RunTrace:
    ...
    for name, desc in [("search", "Search documents"), ("create", "Create record"), ("delete", "Delete record"), ("list", "List records")]:
        status = AffordanceStatus.INTERACTED if name == "search" else AffordanceStatus.DISCOVERED
        trace.affordances.append(Affordance(
            name=name, kind="tool", status=status, relevant=name == "search",
            notes=desc,
        ))

    steps_data = [
        ("Listing available tools", "list_tools", "read", True, ["4 tools available"]),
        ("Inspecting search tool schema", "inspect_tool", "read", True, ["search takes query param"]),
